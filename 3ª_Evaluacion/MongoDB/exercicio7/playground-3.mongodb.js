...
db.getCollection('pokemon').updateOne({"name.english":"Squirtle"}, {$set :{"Descripcion": "Squirtle's shell is not merely used for protection"}})
...
db.getCollection('pokemon').find({"base.Attack":{$lte:20}})

//Show a list with the english name of all Pokemons with attack 50 or greater and type Poison.

db.getCollection('pokemon').find({"base.Attack":{$gte:50}, "type":"Poison"})
...
db.getCollection('pokemon').updateMany(
  ...
  {
    $set: {
      "Attacks": ["strong attack", "ultra attack"]
    }
  }
)
